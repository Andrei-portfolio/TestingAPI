...
//
        ...
        /* После запуска данного теста компания из БД почему то не удаляется. А чтобы убедиться в её удалении
        нужно дебажить вот этот код -- assertNotNull(entityManager.find(CompanyEntity.class, id));
        Сначала программа создаёт компанию, на дебаже мы видем её id. Проверяем в БД. После того, как
        запустим оставшуюся часть кода и обновим БД, данная компания удалиться. Только так, в лекции
        тоже самое приведено от 11.02 где то после 1 час. 50 мин лекции */
